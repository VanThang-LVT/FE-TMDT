...
import './AdminLayout.css'; // Reusing the same CSS from Admin
      ...
      <aside className="admin-sidebar">
        <div className="admin-sidebar-header">
          <div className="admin-sidebar-brand">
            <div className="admin-logo-box" style={{ background: 'var(--success-glow)', color: 'var(--success-color)' }}>
        ...
        <nav className="admin-nav">
          <a className={`admin-nav-item ${location.pathname === '/seller' ? 'active' : ''}`} onClick={() => navigate('/seller')} title="Tổng quan">
            <span className="material-symbols-outlined">dashboard</span>
            {!isSidebarCollapsed && <span>Tổng quan</span>}
          </a>
          <a className={`admin-nav-item ${location.pathname === '/seller/products' ? 'active' : ''}`} onClick={() => navigate('/seller/products')} title="Quản lý sản phẩm">
            <span className="material-symbols-outlined">inventory_2</span>
            {!isSidebarCollapsed && <span>Quản lý sản phẩm</span>}
          </a>
          <a className={`admin-nav-item ${location.pathname === '/seller/inventory' ? 'active' : ''}`} onClick={() => navigate('/seller/inventory')} title="Quản lý tồn kho">
            <span className="material-symbols-outlined">warehouse</span>
            {!isSidebarCollapsed && <span>Quản lý tồn kho</span>}
          </a>
          ...
          <div className="admin-topbar-right">
            ...
            <div className="admin-profile">
              <div className="admin-profile-text">
                <p className="admin-profile-name">{user?.fullName || 'Người bán'}</p>
                <p className="admin-profile-role">Chủ gian hàng</p>
              </div>
              <div className="admin-avatar" style={{ background: 'var(--success-glow)', color: 'var(--success-color)' }}>
                {user?.fullName ? user.fullName.substring(0, 2).toUpperCase() : 'SE'}
              </div>
            </div>
          </div>
        </header>
